...
def evaluate_all_seen(model, test_full, test_labels, seen_indices, anchors_tensor, anchor_keys, device):
    idxs = [i for i, lbl in enumerate(test_labels) if lbl in seen_indices]
    if len(idxs) == 0:
        return 0.0
...
def evaluate_task_seen_anchors(model, test_full, test_labels, task_class_inds, seen_indices, anchors_tensor, device):
    idxs = [i for i, lbl in enumerate(test_labels) if lbl in task_class_inds]
    if len(idxs) == 0:
        return 0.0
    loader = DataLoader(Subset(test_full, idxs), batch_size=128, shuffle=False, num_workers=2)
    model.eval()
...
def zero_shot_eval(model, anchors_tensor, unseen_indices, test_full, test_labels, device):
    if len(unseen_indices) == 0:
        return 0.0
    idxs = [i for i, l in enumerate(test_labels) if l in unseen_indices]
    loader = DataLoader(Subset(test_full, idxs), batch_size=128, shuffle=False, num_workers=2)
...
def linear_probe_unseen(model, train_full, train_labels, test_full, test_labels, unseen_indices, device):
    model.eval()

    train_idx = [i for i, l in enumerate(train_labels) if l in unseen_indices]
    test_idx = [i for i, l in enumerate(test_labels) if l in unseen_indices]
...
def main(cfg):
    ...
    tasks, class_names = make_gtsrb_tasks(cfg['num_tasks'], cfg['batch_size'], augment=True)
    ...
    test_full = GTSRB(root="data", split='test', download=True, transform=test_transform)
    train_full = GTSRB(root="data", split='train', download=True, transform=test_transform)

    test_labels = [lbl for _, lbl in test_full._samples]
    train_labels = [lbl for _, lbl in train_full._samples]
    ...
    for t, (train_loader, test_loader, class_inds) in enumerate(tasks):
        ...
        acc_all_seen = evaluate_all_seen(model, test_full, test_labels, seen_inds, anchors_tensor, anchor_keys, device)
        ...
        if len(unseen_inds) > 0:
            zs = zero_shot_eval(model, anchors_tensor, unseen_inds, test_full, test_labels, device)
            zero_shot_history.append(zs)
            print(f"Zero-shot acc on unseen classes after task {t}: {zs:.4f}")

            lp_acc = linear_probe_unseen(model, train_full, train_labels, test_full, test_labels, unseen_inds, device)
        ...
        for i_task, (_, _, t_classes) in enumerate(tasks):
            if i_task > t:
                ...
                acc_old_task = evaluate_task_seen_anchors(model, test_full, test_labels, t_classes, seen_inds,
