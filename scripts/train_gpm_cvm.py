    ...
    for t, (train_loader, test_loader, class_inds) in enumerate(tasks):
        ...
        for epoch in range(cfg['epochs_per_task']):
            ...
            for images, raw_images, labels in train_loader:
                ...
                if t > 0 and len(feature_mat) > 0:
                    kk = 0
                    for name, module in conv_modules:
                        param = module.weight
                        if param.requires_grad and param.grad is not None:
                            sz = param.grad.data.size(0)
                            proj_mat = feature_mat[kk]

                            grad_flat = param.grad.data.view(sz, -1)
                            param.grad.data -= torch.mm(grad_flat, proj_mat).view(param.size())
                            kk += 1
        ...
        print(f"\n>> GPM task 0 - task {t}...")
        mat_list = get_representation_matrix(model, conv_modules, device, train_loader)
        feature_list = update_GPM(mat_list, threshold=gpm_threshold, feature_list=feature_list)

        feature_mat = []
        for U in feature_list:
            Uf = torch.Tensor(np.dot(U, U.T)).to(device)
            feature_mat.append(Uf)
        print(">> GPM construction done\n")
